...
internal class PlayerMediaSourceFactory(private val context: Context) {
    ...
    fun createMediaSource(
        ...
        val mediaSource = when {
            // Upstream's gated path (restored). CEA-608/708 captions are still exposed: both
            // HlsMediaSource.Factory here AND DefaultMediaSourceFactory's internal HLS path use
            // DefaultHlsExtractorFactory with exposeCea608WhenMissingDeclarations=true by default,
            // so no custom extractor factory is needed. (Forcing all HLS through HlsMediaSource.Factory
            // regressed non-live content — e.g. Torrentio fMP4 movies hit "top bit not zero" in the
            // MP4 box parser — so we keep the original routing and rely on the CC labeling in
            // PlayerRuntimeControllerTracks to surface CEA-608.)
