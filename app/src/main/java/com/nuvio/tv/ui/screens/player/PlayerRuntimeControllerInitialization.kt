            ...
            _exoPlayer?.apply {
                ...
                addListener(object : Player.Listener {
                    ...
                    override fun onPlayerError(error: PlaybackException) {
                        ...
                        // upstream: if the codec crashed while the app is in the BACKGROUND (another
                        // app reclaimed the hardware decoder), don't run the retry chain — each retry
                        // re-acquires a decoder the foreground app reclaims again, burning the budget
                        // and landing on an unrecoverable DECODING_FAILED. Save position, free the
                        // decoder, rebuild paused on resume. Checked first so it wins while backgrounded.
                        ...
                        // usa-tv-next: Live HLS drift — ERROR_CODE_BEHIND_LIVE_WINDOW (1002) fires when
                        // playback falls behind the available segment window (after a buffer stall on a
                        // live channel — the "source:1002, works if replayed" symptom). Recover by
                        // jumping to the live edge + re-preparing instead of surfacing the error.
                        // Time-throttled so a genuinely-unreachable live edge can't hot-loop (then it
